...
class Renderer2D(w: Int, h: Int) extends Renderer(w,h){
  ...
  def calcIndex(x: Int, y: Int): Int = y * (w + 1) + x
  ...
  def setPixel(x: Int, y: Int, char: Char): Unit = {
    try{
      ...
      case e: ArrayIndexOutOfBoundsException 
        => println("Invalid array index at setPixel. x:" + x + ", y:" + y + ", char:" +char)
    }
  }
  
  def drawLine(p1: Point2D, p2: Point2D,color: Int): Unit = drawLine(p1.x, p1.y,p2.x,p2.y,color)
    ...
    if (Math.abs(y2 - y1) > Math.abs(x2 - x1)) {  
      ...
      //if ((x3 - x4) > 0) {steepline(x4, y4, x3, y3,color); return;}  
  
      var x = x3; var y = y3; var sum = x4 - x3; var Dx = 2 * (x4 - x3); var Dy = Math.abs(2 * (y4 - y3));  
...
//        steepline(y1, x1, y2, x2,color); return;   
    }else{ 
      var x = x1; var y = y1; var sum = x2 - x1; var Dx = 2 * (x2 - x1); var Dy = Math.abs(2 * (y2 - y1));  
  ...
  def renderRectangle(rect: Rectangle2D, loc: Vec3){
    ...
    if(rect.defFill){
      for(x <- p1.x to p2.x){
        for(y <- p1.y to p2.y){
          setPixel(x,y,Renderer2D.chars(rect.color1))
        }
      }
  ...
  def renderImage(image: Image2D, loc: Vec3){
    ...
    for(imgY <- 0 until img.length){
      for(imgX <- 0 until img(imgY).length){
        if(!(fill && img(imgX)(imgY) != 0)){ // if fill is false, char 0 is transparent
  ...
  def renderTextRectangle(tRect: TextRect2D, loc: Vec3){
    val fill = tRect.rect.defFill
    val text = tRect.text
    val p1 = new Point2D(loc.x.toInt, loc.y.toInt)
    val p2 = new Point2D(loc.x.toInt + tRect.rect.w, loc.y.toInt + tRect.rect.h)
    val offX = tRect.offX
    val offY = tRect.offY
    
    if(fill){
      renderRectangle(tRect.rect, loc)
    }
    
    var lines  = text.split('\n')  
    var lineCounter = 0
    
    for(y <- p1.y+offY to Math.min(p2.y, lines.length) ){    
      var charCounter = 0
      
      for(x <- p1.x+offX to Math.min(p2.x,p1.x+lines(lineCounter).length()-1)){
  ...
  def display: String = this.frameBuffer.mkString("")
